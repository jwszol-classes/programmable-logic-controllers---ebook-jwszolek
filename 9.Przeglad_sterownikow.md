...
Kontrolery PACSystems RX3i mogą być programowane w języku LD, ST, C oraz FBD. Do dyspozycji mamy szereg gotowych funkcji wspierających pisanie aplikacji, między innymi testowanie zmian programu na ruchu, zmienne symboliczne, bloki funkcyjne z parametrami, adresowanie pośrednie. RX3i pozwala także na programowanie na ruchu.

## PACSYSTEMS RX7I

![Photo of the VersaMax Nano,width=300](http://przemyslowa-automatyka.com.pl/media/catalog/category/pacrx7.jpg)

Seria RX7I systemu PLC to kolejna wersja sterowników dla systemu PAC. Tak samo jak sterowniki RX3I, wyposażone są one w uniwersalne środowisko programowania, co gwarantuje mobilność aplikacji na różnego rodzaju platformach sprzętowych. Urządzenie jest wyposażone w dwa porty szeregowe, jeden port RS-232, jeden port RS-235 oraz porty Ethernet. Dzięki swojej konstrukcji, urządzenie może pracować w różnych środowiskach sieciowych. Pamięć użytkownika zastosowana w sterownikach tej serii może wynosić maksymalnie 10 megabajtów. Sterowniki RX7I są przeznaczone do sterowania małymi i średnimi aplikacjami przemysłowymi wymagającymi rozbudowanych możliwości komunikacyjnych i obsługi na ruchu. Kontrolery rodziny PACSystems RX7i to zaawansowane technicznie i funkcjonalnie rozwiązania firmy GE Intelligent Platforms, przeznaczone do obsługi najbardziej rygorystycznych instalacji automatyki, w których wymagana jest bardzo szybka reakcja na zmiany pojawiające się na obiekcie. Ogromne możliwości funkcjonalne, uniwersalne środowisko programistyczne oraz wysoka wydajność sprawiają, że RX7i idealnie nadaje się do zintegrowanych, kompletnych linii produkcyjnych wymagających otwartej architektury i pracy w rozproszeniu.

### Zaawansowane możliwości programistyczne

Kontrolery serii RX7i programowane i konfigurowane są za pomocą oprogramowania narzędziowego Proficy Machine Edition w wersji Proffesional, pozwalającego na szybkie projektowanie aplikacji wykorzystując wszystkie dostępne funkcje sterownika. Tworzenie algorytmu sterowania realizowane jest w sposób on-line z wykorzystaniem funkcji testowania zmian programu na ruchu bez wyłączania/zatrzymywania całego systemu. Powrót do poprzedniej wersji aplikacji możliwy jest poprzez jedno kliknięcie, dzięki czemu w przypadku niewłaściwej pracy obiektu możliwy jest błyskawiczny powrót do ostatniej, pewnej wersji aplikacji. Obsługa zmiennych symbolicznych, bloków parametrycznych i funkcyjnych z możliwością dowolnego definiowania struktury (UDFB) to tylko niektóre funkcje pozwalające na pisanie rozbudowanych algorytmów sterowania w językach C, FBD, LD oraz ST. Przydatna jest również możliwość tworzenia własnych bibliotek z gotowymi fragmentami kodu i wykorzystywanie ich przy następnych projektach. Oprogramowanie wspiera również proces uruchomienia systemu oraz jego późniejszej diagnostyki i serwisowania.
