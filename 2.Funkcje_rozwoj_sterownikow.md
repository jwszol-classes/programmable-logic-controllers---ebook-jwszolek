...
#### Dwie podstawowe koncepcje w budowaniu sterowników PLC to:
- Konstrukcja blokowa (kompaktowa): 
  - Zwykle stosowana do sterowników małych (do 256 wejść/wyjść)
  - Zawierają w sobie kompletny moduł sterownika 
    - Zasilacz 
    - Jednostka centralna (CPU)
    - Moduły wejść oraz wyjść o ustalonej liczbie zacisków 
  - Prosta konstrukcja 
  - Łatwy montaż
- Konstrukcja panelowa (modułowa):
  - Może być stosowana do wszystkich wielkości sterowników, jednak przeważa pośród sterowników średnich (do 1024 wejść/wyjść) oraz dużych (powyżej 1024 wejść/wyjść) 
  - Poszczególne moduły funkcjonalne takie jak:
      - Zasilacz 
      - CPU
      - Moduły wejść i wyjść
  - Są umieszczone w oddzielnych panelach 
  - Użytkownik konfiguruje strukturę sterownika poprzez dobór paneli i umieszczenie ich w obudowie
  - Moduły rozszerzające zawierające dodatkowe układy wejść i wyjść korzystają z pamięci i zasilacza w jednostce centralnej
